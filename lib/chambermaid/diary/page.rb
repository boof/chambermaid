...
class Chambermaid::Diary::Page
  ...
  alias_method :__set_ivar, :instance_variable_set
  instance_methods.each { |meth| undef_method(meth) unless meth =~ /\A__/ }
  include Grit

  def initialize(about, path)
    @about, @loaded, @repository = about, false, Repo.new(path)
    @constructor = @about.constructor || proc { |about, tree|
  ...
  protected
    ...
    def __bind_context(method, context)
      (class << self; self; end).
          ...
          DEF
      __set_ivar :"@__context_of_#{ method }", context
    end
    def __load_target
      return unless defined? @loaded
      @target, @loaded = @constructor[@about, @repository.tree], true
    end
